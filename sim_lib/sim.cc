...
int main(int argc, const char* argv[]) try {
  ...
  app.add_option("elf", elfPath, "Path to executable ELF file")->required()->check(CLI::ExistingFile);
  ...
  fmt::println("MIPS: {:.2f}", hart.getIcount() / (duration.count() * 1'000'000));

  return EXIT_SUCCESS;
} catch (const std::exception& ex) {
  fmt::println(std::cerr, "Caught exception of type {}: {}", typeid(ex).name(), ex.what());
  return EXIT_FAILURE;
} catch (...) {
